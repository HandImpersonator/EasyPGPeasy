...
def df(dec, event, key, lang, mes_dec_eng, mes_dec_esp, values):
    ...
    if not values["input"].strip():
        ...
        try:
            out_dec, fin = all_imp.edsv.pgpy_decrypt(values["input"].strip(), False, key, lang, dec)
...
def dm(dec, event, key, lang, mes, mes_dec_eng, mes_dec_esp):
    ...
    if event == 24:
        ...
        if all_imp.os.path.isfile(mes):
            try:
                _, fin = all_imp.edsv.pgpy_decrypt(mes, False, key, lang, dec)
            except all_imp.pgpy.errors.PGPDecryptionError:
                error_pass_eng = ["The passphrase is incorrect.", "Returning to menu.", "Error decrypting!"]
                error_pass_esp = ["La contraseña es incorrecta.", "Volviendo al menú.", "¡Error descifrando!"]
                if lang == "eng":
            ...
            no_mes_eng = ["No message to decrypt found in ./Imported.", "Error decrypting!"]
            no_mes_esp = ["No existe un mensaje para descifrar en ./Imported.", "¡Error descifrando!"]
            if lang == "eng":
                error1 = no_mes_eng[0]
                error2 = no_mes_eng[1]
            elif lang == "esp":
        ...
        if fname:
            ...
            if extension == "pgp.PK":
                try:
                    _, _ = all_imp.edsv.pgpy_decrypt(fname, True, key, lang, dec)
                except all_imp.pgpy.errors.PGPDecryptionError:
                    error_pass_eng = ["The passphrase is incorrect.", "Returning to menu.", "Error decrypting!"]
                    error_pass_esp = ["La contraseña es incorrecta.", "Volviendo al menú.", "¡Error descifrando!"]
                    if lang == "eng":
